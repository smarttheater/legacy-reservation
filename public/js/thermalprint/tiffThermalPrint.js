...
window.tiffThermalPrint = (function(d,StarWebPrintBuilder,StarWebPrintTrader){
    ...
    //印刷用TIFFロゴ画像(画像ファイルをCanvasで読んでContextをStarWebPrintBuilder.createBitImageElementで変換して得た印刷データ文字列)
    ...
    var genRequestByReservationObj = function(reservation){
        ...
        try{
            ...
            request += builder.createTextElement({
                emphasis: true,
                data: reservation.ticket_type_name_ja+'/'+reservation.ticket_type_charge+'(税込)\n'
            });
            //強調を解除して英語券種・金額
            request += builder.createTextElement({
                emphasis: false,
                data: reservation.ticket_type_name_en+'/'+reservation.ticket_type_charge+'(tax incl.)\n\n'
            });
