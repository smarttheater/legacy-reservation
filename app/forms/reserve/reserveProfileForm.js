...
exports.default = (req) => {
    ...
    req.checkBody('lastName', req.__('MaxLength{{fieldName}}{{max}}', { fieldName: req.__('LastName'), max: NAME_MAX_LENGTH.toString() })).isLength({
        max: NAME_MAX_LENGTH
    });
    req.checkBody('lastName', req.__('Invalid{{fieldName}}', { fieldName: req.__('LastName') })).matches(/^[ァ-ロワヲンーa-zA-Z]*$/);
    // firstName
    req.checkBody('firstName', req.__('NoInput{{fieldName}}', { fieldName: req.__('FirstName') })).notEmpty();
    req.checkBody('firstName', req.__('MaxLength{{fieldName}}{{max}}', { fieldName: req.__('FirstName'), max: NAME_MAX_LENGTH.toString() })).isLength({
        max: NAME_MAX_LENGTH
    });
    ...
    req.checkBody('email', req.__('Match{{fieldName}}', { fieldName: req.__('Email') })).equals(`${req.body.emailConfirm}@${req.body.emailConfirmDomain}`);
    // emailConfirm
    req.checkBody('emailConfirm', req.__('NoInput{{fieldName}}', { fieldName: req.__('EmailConfirm') })).notEmpty();
    // emailConfirmDomain
    req.checkBody('emailConfirmDomain', req.__('NoInput{{fieldName}}', { fieldName: req.__('EmailConfirmDomain') })).notEmpty();
    // address
