            ...
            reservationModel.remove(() => {
                ...
                this.setProcessLogger(reservationModel.paymentNo, () => {
                    ...
                    if (process.env.NODE_ENV === 'prod' || process.env.NODE_ENV === 'test') {
                        this.res.locals.retURL = `${this.req.protocol}://${conf.get('dns_name_for_gmo_result')}${this.router.build('gmo.reserve.result')}?locale=${this.req.getLocale()}`;
                        // 決済キャンセル時に遷移する加盟店URL
                        this.res.locals.cancelURL = `${this.req.protocol}://${conf.get('dns_name_for_gmo_result')}${this.router.build('gmo.reserve.cancel', { paymentNo: reservationModel.paymentNo })}?locale=${this.req.getLocale()}`;
                    }
                    else {
