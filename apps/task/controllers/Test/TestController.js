...
const GMONotificationModel_1 = require('../../../frontend/models/Reserve/GMONotificationModel');
...
class TestController extends BaseController_1.default {
    ...
    receiveGMONotify() {
        ...
        let gmoNotificationModel = GMONotificationModel_1.default.parse(body);
     ...
     * GMO結果通知を処理する
     */
    processGMONotification() {
        mongoose.connect(MONGOLAB_URI, {});
        this.logger.info('finding notification...');
        Models_1.default.GMONotification.findOne({
            processed: false
        }, (err, notification) => {
            ...
            Models_1.default.Reservation.find({
                ...
                // チェック文字列
                let shopPassString = GMOUtil_1.default.createShopPassString(notification.get('shop_id'), notification.get('order_id'), notification.get('amount'), conf.get('gmo_payment_shop_password'), moment(reservations[0].get('purchased_at')).format('YYYYMMDDHHmmss'));
                ...
                // クレジットカード決済の場合
                if (notification.get('pay_type') === GMOUtil_1.default.PAY_TYPE_CREDIT) {
                    switch (notification.get('status')) {
                        case GMOUtil_1.default.STATUS_CREDIT_CAPTURE:
                            ...
                            Models_1.default.Reservation.update({ payment_no: notification.get('order_id') }, {
                                status: ReservationUtil_1.default.STATUS_RESERVED,
                                updated_user: 'system'
                            }, { multi: true }, (err, raw) => {
                ...
                else if (notification.get('pay_type') === GMOUtil_1.default.PAY_TYPE_CVS) {
                    switch (notification.get('status')) {
                        ...
                        Models_1.default.Reservation.find({
                            ...
                            let promises2 = reservations.map((reservation) => {
                                return new Promise((resolve2, reject2) => {
                                    ...
                                    if (change.pay_type === GMOUtil_1.default.PAY_TYPE_CREDIT && reservation.get('payment_method') === GMOUtil_1.default.PAY_TYPE_CVS) {
                                        update['charge'] = reservation.get('charge') - 150;
                                    }
                                    else if (change.pay_type === GMOUtil_1.default.PAY_TYPE_CVS && reservation.get('payment_method') === GMOUtil_1.default.PAY_TYPE_CREDIT) {
                                        update['charge'] = reservation.get('charge') + 150;
                                    }
                                    if (searchTradeResult.Status === GMOUtil_1.default.STATUS_CREDIT_CAPTURE || searchTradeResult.Status === GMOUtil_1.default.STATUS_CVS_PAYSUCCESS) {
