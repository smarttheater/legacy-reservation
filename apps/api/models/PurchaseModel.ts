...
export default class PurchaseModel {
    ...
    public save(cb: () => void, ttl?: number) {
        let key = PurchaseModel.getRedisKey(this.token);
     ...
     */
    public static find(token: string, cb: (err: Error | void, reservationModel: PurchaseModel) => void): void {
        let key = PurchaseModel.getRedisKey(token);
        redisClient.get(key, (err, reply) => {
            if (err) return cb(err, null);
            if (reply === null) return cb(new Error('Not Found'), null);

            let reservationModel = new PurchaseModel();
